...
try:
    delivery_partners_df = (
        spark.read
            .format("parquet")
            .load(file_path)
            .withColumn("partner_id", col("partner_id").cast("string"))
...
new_df = delivery_partners_df.join(
    existing_files,
    on="_source_file",
    how="leftanti"
)
...
incoming_schema = delivery_partners_df.schema.json()
...
row_count = delivery_partners_df.count()
...
mandatory_cols = ("partner_id", "partner_name", "city", "rating", "updated_at")

missing = [c for c in mandatory_cols if c not in delivery_partners_df.columns]
...
validated_df = (
    ...
    .withColumn(
        "quarantine_reason",
        when(col("partner_id").isNull(), "NULL_PARTNER_ID")
        .when(trim(col("partner_id")) == "", "EMPTY_PARTNER_ID")
