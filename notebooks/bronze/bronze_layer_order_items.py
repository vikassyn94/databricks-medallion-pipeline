...
incoming_schema = order_items_df.schema.json()
...
mandatory_cols = ("order_item_id", "order_id", "restaurant_id", "item_name", "quantity", "price", "updated_at")

missing = [c for c in mandatory_cols if c not in order_items_df.columns]
...
validated_df = (
    ...
    .withColumn(
        "quarantine_reason",
        when(col("order_item_id").isNull(), "NULL_ORDER_ITEM_ID")
        .when(trim(col("order_item_id")) == "", "EMPTY_ORDER_ITEM_ID")
        .when(col("order_id").isNull(), "NULL_ORDER_ID")
        .when(trim(col("order_id")) == "", "EMPTY_ORDER_ID")
