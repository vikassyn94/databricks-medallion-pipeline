...
print(f"Running Silver for batch_id: {batch_id}")
    ...
    try:
        ...
        if last_phase is None or last_phase < 4:
            ...
            silver_input_df = (
                ...
                .withColumn(
                    ...
                    .when(trim(col("customer_id")) == "", "customer_id is empty")
                    .when(trim(col("order_date")) == "", "order_date is empty")
                    .when(col("order_date").isNull(), "order_date is null")
                    .when(col("order_date").cast("date") > col("parsed_date"), "order_date is in future")
                    .when(col("amount").isNull(), "amount is null")
                    .when(trim(col("amount")) == "", "amount is empty")
                    .when(col("amount").cast("double") <= 0, "amount is invalid")
                    .when(col("payment_status").isNull(), "payment_status is null")
                    .when(trim(col("payment_status")) == "", "payment_status is empty")
                    .when(col("order_status").isNull(), "order_status is null")
                    .when(trim(col("order_status")) == "", "order_status is empty")
                    .when(col("restaurant_id").isNull(), "restaurant_id is null")
                    .when(trim(col("restaurant_id")) == "", "restaurant_id is empty")
                    .when((col("order_status") == 'COMPLETED') & (col("partner_id").isNull()), "partner_id is null")
                    .when((col("order_status") == 'COMPLETED') & (trim(col("partner_id")) == ""), "partner_id is empty")
                )
            )
        ...
        if last_phase is None or last_phase < 7:
            ...
            analysis_df = (
                ...
                .withColumn(
                    "operation",
                    when(col("t.order_id").isNull(), "INSERT")
        ...
        if last_phase is None or last_phase < 8:
            ...
            (
                ...
                .merge(
                    merge_df.alias("s"), "t.order_id = s.order_id" 
                )
