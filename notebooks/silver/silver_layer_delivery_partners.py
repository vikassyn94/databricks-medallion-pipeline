...
def process_batch(batch_id):
    try:
        ...
        if last_phase is None or last_phase < 4:
            ...
            silver_input_df = (
                ...
                .withColumn(
                    ...
                    .when(col("rating").isNull(), "rating is null")
                    .when(trim(col("rating")) == "", "rating is empty")
                    .when((col("rating").isNotNull()) & ((col("rating") < 0) | (col("rating") > 5)), "non-digit rating")
                )
            )
        ...
        if last_phase is None or last_phase < 6:
            window_spec = Window.partitionBy("partner_id")\
        ...
        if last_phase is None or last_phase < 7:
            ...
            analysis_df = (
                ...
                .withColumn(
                    "operation",
                    when(col("t.partner_id").isNull(), "INSERT")
        ...
        if last_phase is None or last_phase < 8:
            ...
            (
                ...
                .merge(
                    merge_df.alias("s"), "t.partner_id = s.partner_id" 
                )
