...
def process_batch(batch_id):
    try:
        ...
        if last_phase is None or last_phase < 4:
            ...
            silver_input_df = (
                ...
                .withColumn(
                    ...
                    .when(col("cuisine").isNull(), "cuisine is null")
                    .when(trim(col("cuisine")) == "", "cuisine is empty")
                )
            )
        ...
        if last_phase is None or last_phase < 6:
            window_spec = Window.partitionBy("restaurant_id")\
        ...
        if last_phase is None or last_phase < 7:
            ...
            analysis_df = (
                ...
                .withColumn(
                    "operation",
                    when(col("t.restaurant_id").isNull(), "INSERT")
        ...
        if last_phase is None or last_phase < 8:
            ...
            (
                ...
                .merge(
                    merge_df.alias("s"), "t.restaurant_id = s.restaurant_id" 
                )
