...
def process_batch(batch_id):
    try:
        ...
        if last_phase is None or last_phase < 4:
            ...
            silver_input_df = (
                ...
                .withColumn(
                    ...
                    .when(trim(col("quantity")) == "", "quantity is empty")
                    .when(col("quantity").isNull(), "quantity is null")
                    .when((col("quantity").isNotNull()) & (regexp_extract(col("quantity"), "^[0-9]+$", 0) == ""), "non-digit quantity")
                    .when((col("quantity").isNotNull()) & (col("quantity").cast("int") < 0), "negative quantity")
                    .when(col("price").isNull(), "price is null")
                    .when(trim(col("price")) == "", "price is empty")
                    .when((col("price").isNotNull()) & (col("price").cast("float") < 0), "negative price")
                )
            )
        ...
        if last_phase is None or last_phase < 6:
            window_spec = Window.partitionBy("order_item_id", "order_id")\
        ...
        if last_phase is None or last_phase < 7:
            ...
            merge_analysis = (
                ...
                .join(
                    silver_df.alias("t"),
                    (col("s.order_item_id") == col("t.order_item_id")) & (col("s.order_id") == col("t.order_id")),
                    "left"
                )
            )
            ...
            analysis_df = (
                ...
                .withColumn(
                    "operation",
                    when((col("t.order_item_id").isNull()) & (col("t.order_id").isNull()), "INSERT")
        ...
        if last_phase is None or last_phase < 8:
            ...
            (
                ...
                .merge(
                    merge_df.alias("s"), "t.order_item_id = s.order_item_id AND t.order_id = s.order_id" 
                )
